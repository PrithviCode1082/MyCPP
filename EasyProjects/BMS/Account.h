...
private:
  ...
  void changeBalance(float amount, string type) {
    ...
    cout << "$ " << amount << " was "
         << ((type == "+") ? "Deposited" : "Withdrawn") << " successfully!\n";
...
public:
  Account(string name, int mode) : userName(name) {
    ...
    cout << ((mode == 0) ? "Current" : "Savings") << " Account - " << name
         << " with Account number " << this->Account_Number
         << " was created successfully\n";
    cout << "Happy Banking - Welcome!\n";
  };
  ~Account() {};
  void deposit(float amount) { changeBalance(amount, "+"); }
  ...
  void AccountInfo() {
    ...
    cout << "Account Type: " << (this->is_saving == 1 ? "Savings" : "Current")
         << endl;
    if (this->is_saving == 1) {
    ...
    for (float transaction : this->transaction_history) {
      cout << "$ " << transaction
           << (transaction > 0 ? " Deposited" : " Withdrawn") << endl;
    }
    cout << "*******************************************\n";
  }
  void getInterest() {
  ...
  int getAccountNum() { return this->Account_Number; }
};
